...
const MojiOglasi = ({ currentUser }) => {
    ...
    const [newProduct, setNewProduct] = useState({
        ...
        pickupCity: "",
        pickupPostalCode: "",
        pickupCountry: "Hrvatska",
        returnAddress: "",
        returnArea: "",
        returnCity: "",
        returnPostalCode: "",
        returnCountry: "Hrvatska",
    });
    ...
    const getImageUrl = (listingId) => {
        ...
        return `${BASE_URL}/merchant/get-image?listingID=${listingId}&token=${token}`;
    };
        ...
        try {
            ...
            if (res && res.ok) {
                ...
                if (newProduct.photoFile) {
                    const imageUploaded = await uploadImage(savedListing.id, newProduct.photoFile);
                    if (!imageUploaded) {
                        alert("Oglas kreiran, ali slika nije uspješno prenesena.");
                    }
                }
                    ...
                    <div className="form-image-preview">
                        ...
                        <input
                            ...
                            onChange={(e) => {
                                ...
                                if (file) {
                                    setNewProduct({...newProduct, photoFile: file});
                                }
                            }}
                        />
                        {newProduct.photoFile && (
                            <div style={{ marginTop: '10px' }}>
                                <img
                                    src={URL.createObjectURL(newProduct.photoFile)}
                                    alt="Preview"
                ...
                {allAds.map((item) => (
                    <div className="card" key={item.id}>
                        <img
                            ...
                            alt={item.title}
                            onError={(e) => {
                                e.target.src = "https://placehold.co/600x400?text=Nema+slike";
                            }}
                        />
